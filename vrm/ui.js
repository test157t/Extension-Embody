...
function applyModelZIndex(zIndex) {
    ...
    const safeValue = Number.isFinite(parsed) ? Math.round(parsed) : 4;
    document.documentElement.style.setProperty('--vrm-canvas-z-index', String(safeValue));

    const canvas = document.getElementById(VRM_CANVAS_ID);
    if (canvas) {
        canvas.style.zIndex = String(safeValue);
    }
}

async function onModelZIndexChange() {
    const rawValue = Number($('#vrm_model_z_index').val());
    const safeValue = Number.isFinite(rawValue) ? Math.round(rawValue) : 4;

    extension_settings.vrm.model_z_index = safeValue;
    $('#vrm_model_z_index').val(safeValue);
    applyModelZIndex(safeValue);
...
async function onHitboxCallmodeDelayChange() {
    const rawValue = Number($('#vrm_hitbox_callmode_sync_delay').val());
    const safeValue = Number.isFinite(rawValue)
        ? Math.max(0, Math.min(3000, Math.round(rawValue)))
        : 220;

    extension_settings.vrm.hitbox_callmode_sync_delay_ms = safeValue;
    $('#vrm_hitbox_callmode_sync_delay').val(safeValue);
    $('#vrm_hitbox_callmode_sync_delay_value').text(safeValue);
    saveSettingsDebounced();
}

async function onHitboxCallmodeWaitTtsStartChange() {
    const rawValue = Number($('#vrm_hitbox_callmode_wait_tts_start').val());
    const safeValue = Number.isFinite(rawValue)
        ? Math.max(0, Math.min(12000, Math.round(rawValue)))
        : 4500;

    extension_settings.vrm.hitbox_callmode_wait_tts_start_ms = safeValue;
    $('#vrm_hitbox_callmode_wait_tts_start').val(safeValue);
    $('#vrm_hitbox_callmode_wait_tts_start_value').text(safeValue);
    saveSettingsDebounced();
}
