export var VAD = function (options) {
    // Default options - tuned for natural speech with pauses
    this.options = {
        fftSize: 512,
        bufferLen: 512,
        voice_stop: function () { },
        voice_start: function () { },
        smoothingTimeConstant: 0.95,      // Slightly less smoothing for faster response
        energy_offset: 1e-8,              // Lower baseline - more sensitive
        energy_threshold_ratio_pos: 2,    // 2x threshold - more sensitive to speech
        energy_threshold_ratio_neg: 0.5,  // Higher neg threshold - harder to stop
        energy_integration: 0.3,          // Slower adaptation to noise
        min_energy: 1e-9,                 // Absolute minimum energy to consider as speech (prevents triggering on silence)
        filter: [
            { f: 85, v: 0 },    // Below 85Hz - ignore rumble
            { f: 200, v: 0.4 }, // Low bass - reduced
            { f: 400, v: 0.9 }, // Voice fundamental
            { f: 2500, v: 1 },  // Core speech formants (strongest)
            { f: 4000, v: 0.8 },// Upper formants
            { f: 6000, v: 0.3 },// High - reduced
            { f: 22050, v: 0 }
        ],
        source: null,
        context: null
    ...
    this.voiceTrend = 0;
    this.voiceTrendMax = 20;
    this.voiceTrendMin = -50;
    this.voiceTrendStart = 2;     // Trigger faster (2 cycles ~20ms)
    this.voiceTrendEnd = -40;     // Require much more silence before stopping
    ...
    // Debouncing - tuned for responsive start, very long hold for natural speech
    this.lastVoiceStart = 0;
    this.lastVoiceStop = 0;
    this.minSpeechDuration = 30;   // 30ms - trigger very quickly
    this.hangoverTime = 1200;      // 1.2s hold - prevents noise between words from triggering false segments
    ...
    this.monitor = function () {
        ...
        // Don't trigger on silence - require minimum absolute energy
        var hasMinEnergy = energy > this.options.min_energy;

        if (hasMinEnergy && signal > this.energy_threshold_pos) {
            this.voiceTrend = (this.voiceTrend + 1 > this.voiceTrendMax) ? this.voiceTrendMax : this.voiceTrend + 1;
        } else if (signal < -this.energy_threshold_neg || !hasMinEnergy) {
            // Treat no energy as silence
        ...
        if (this.voiceTrend > this.voiceTrendStart) {
            ...
            // Only trigger start after minimum speech duration
            ...
            // Only trigger end after hangover time
            if (now - this.silenceStartTime >= this.hangoverTime) {
                end = true;
            }
        } else {
            // In between - reset timers slowly
            if (this.voiceTrend <= 0) {
                this.speechStartTime = 0;
            }
            if (this.voiceTrend >= 0) {
